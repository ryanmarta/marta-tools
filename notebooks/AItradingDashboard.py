...
with st.sidebar:
    ...
    project_selection = st.radio("Available Projects", ["📈 AI Quant Trading", "🧠 ML Volatility (Regime Detection)", "HUNTER!"])
    st.markdown("---")
...
# ==========================================
elif project_selection == "HUNTER!":
    st.title("🤖 Nexus Scanner: S&P 100 Elite")
    st.caption("Automated Opportunity Hunter | Criteria: Ryan Model + Trap Detection")
    ...
    TICKER_LIST = [
        ...
        "SPY", "QQQ", "IWM", "DIA", "TLT","APH","MSTR","F","FORD"
    ]
    ...
    def batch_process_tickers(tickers):
        ...
        for symbol in tickers:
            try:
                ...
                highs = get_series(data_batch, symbol, 'High').dropna()
                lows = get_series(data_batch, symbol, 'Low').dropna()
                spy_closes = get_series(data_batch, "SPY", 'Close').dropna()
                
                if len(closes) < 60: continue

                # --- 1. RYAN MODEL METRICS ---
                ...
                if day_range == 0: range_position = 0.5 
                else: range_position = (current_price - curr_low) / day_range

                if range_position > 0.8: intraday_health = "POWER" 
                elif range_position < 0.2: intraday_health = "WEAK" 
                elif range_position < 0.5 and trend_bias == "BULLISH": intraday_health = "FADING" 
                else: intraday_health = "NEUTRAL"
                ...
                # Penalty for Fading
                if intraday_health == "FADING" or intraday_health == "WEAK":
                    confidence -= 15 
                ...
                results.append({
                    "Ticker": symbol,
                    "Price": current_price,
                    "Action": action_signal, # New Column
                    "Trend": trend_bias,
                    "Momentum": ema_signal,
    ...
    if st.button("🚀 Initialize Scan Sequence"):
        ...
        with st.spinner(f"Scanning {len(TICKER_LIST)} Assets..."):
            ...
            if df_results is not None and not df_results.empty:
                ...
                longs = df_results[(df_results['Trend'] == 'BULLISH') & (df_results['Momentum'] == 'BULLISH') & (df_results['Squeeze'] == 'COILED') & (df_results['Confidence'] >= min_confidence)]
                shorts = df_results[(df_results['Trend'] == 'BEARISH') & (df_results['Momentum'] == 'BEARISH') & (df_results['Squeeze'] == 'COILED') & (df_results['Confidence'] >= min_confidence)]

                # 🟢 LONG CANDIDATES
                st.subheader(f"🟢 Long Setups ({len(longs)})")
                if not longs.empty:
                    st.dataframe(
                        longs.style.format({"Price": "${:.2f}", "Bandwidth": "{:.4f}", "RS_vs_SPY": "{:.2%}"}),
                    ...
                    st.info(f"No Long setups found matching criteria (Conf > {min_confidence}% + Coiled).")
                ...
                if not shorts.empty:
                    st.dataframe(
                        shorts.style.format({"Price": "${:.2f}", "Bandwidth": "{:.4f}", "RS_vs_SPY": "{:.2%}"}),
                        use_container_width=True,
                        column_config={
                            "Confidence": st.column_config.ProgressColumn("Confidence Score", format="%d%%", min_value=0, max_value=100)
                        },
                    ...
                    st.info(f"No Short setups found matching criteria (Conf > {min_confidence}% + Coiled).")
                ...
                **1. The Action Column**
                * **✅ BUY:** The "Perfect Storm." **Confidence > 80%** + **POWER** health + **Bullish** Trend. 
                * **WAIT:** Good setup, but missing the "Power" finish. Wait for tomorrow.
                
                **2. Health Status (Intraday Reality Check)**
                * **🟢 POWER:** Price is closing near the High of the Day (Top 20%). *Strong Conviction.*
                * **🟡 NEUTRAL:** Price is chopping in the middle of the daily range. *Wait and See.*
                * **🔴 FADING:** Price popped up but is now dropping back (Trap Warning). *Avoid.*
                * **🔴 WEAK:** Price is closing near the Low of the Day. *Bearish.*

                **3. Why Buy High?**
                * We buy "POWER" because it proves institutions are holding the price up into the close.
                * If we bought the "dip" (FADING), we might be catching a falling knife.
                """)
